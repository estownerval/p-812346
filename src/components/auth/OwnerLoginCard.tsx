    ...
    try {
      ...
      if (authData.user) {
        toast({
          ...
          description: "Welcome, Establishment Owner!",
          duration: 3000,
        });
        
        // Fetch user profile to ensure they have the correct role
        ...
        if (profileData?.role === 'establishment_owner') {
          navigate("/owner/dashboard");
        } else {
          // If the user has a different role, redirect them accordingly
          if (profileData?.role === 'admin') {
            navigate("/admin/dashboard");
          } else if (profileData?.role === 'fire_inspector') {
            navigate("/inspector/dashboard");
          } else {
            navigate("/owner/dashboard"); // Default to owner dashboard if role is not recognized
          }
        }
      }
    } catch (error: any) {
    ...
    <div className="w-[605px] border flex flex-col items-center bg-white p-10 rounded-[20px] border-solid border-[#524F4F] max-md:w-[90%] max-md:max-w-[605px] max-sm:p-5">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-[498px]">
        ...
        <div className="text-center mt-5">
          <span className="text-black text-base font-medium">Don't have an account? </span>
